...
func (f *fakeTriageFetcher) GetIssue(_ context.Context, _ string) (jira.Issue, string, error) {
...
func (f *fakeTriageRetriever) Search(_ context.Context, _ string, _ string, _ int) ([]handlers.Hit, error) {
	return f.hits, f.err
}
...
func TestTicketTriage_PriorityHeuristic(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			f := &fakeTriageFetcher{issue: jira.Issue{Key: "ABC-1", Summary: tc.summary}, desc: tc.desc}
...
func TestTicketTriage_TeamFrequency(t *testing.T) {
	f := &fakeTriageFetcher{issue: jira.Issue{Key: "ABC-1", Summary: "x"}}
	r := &fakeTriageRetriever{hits: []handlers.Hit{
		{IssueKey: "ABC-2", Excerpt: excerptWithAssignee("Alice")},
		{IssueKey: "ABC-3", Excerpt: excerptWithAssignee("Alice")},
		{IssueKey: "ABC-4", Excerpt: excerptWithAssignee("Bob")},
	}}
	h := handlers.TicketTriage(f, r)
	out, err := h(context.Background(), handlers.TicketTriageInput{IssueKey: "ABC-1", ProjectKey: "ABC"})
	require.NoError(t, err)
...
func TestTicketTriage_TeamTieFirstAppearance(t *testing.T) {
	...
	r := &fakeTriageRetriever{hits: []handlers.Hit{
		{IssueKey: "ABC-2", Excerpt: excerptWithAssignee("Alice")},
		{IssueKey: "ABC-3", Excerpt: excerptWithAssignee("Bob")},
	}}
	h := handlers.TicketTriage(f, r)
	out, err := h(context.Background(), handlers.TicketTriageInput{IssueKey: "ABC-1", ProjectKey: "ABC"})
	require.NoError(t, err)
...
func TestTicketTriage_NoAssignees(t *testing.T) {
	f := &fakeTriageFetcher{issue: jira.Issue{Key: "ABC-1", Summary: "x"}}
	r := &fakeTriageRetriever{hits: []handlers.Hit{
		{IssueKey: "ABC-2", Excerpt: "no assignee here"},
	}}
	h := handlers.TicketTriage(f, r)
	out, err := h(context.Background(), handlers.TicketTriageInput{IssueKey: "ABC-1", ProjectKey: "ABC"})
	require.NoError(t, err)
...
func TestTicketTriage_FiltersSelfMatch(t *testing.T) {
	f := &fakeTriageFetcher{issue: jira.Issue{Key: "ABC-1", Summary: "login"}}
	r := &fakeTriageRetriever{hits: []handlers.Hit{
		{IssueKey: "ABC-1", Excerpt: excerptWithAssignee("Self")},
		{IssueKey: "ABC-2", Excerpt: excerptWithAssignee("Alice")},
	}}
	h := handlers.TicketTriage(f, r)
	...
	require.Equal(t, "ABC-2", out.SimilarIssues[0].IssueKey)
	require.Equal(t, "Alice", out.SuggestedTeam)
}
